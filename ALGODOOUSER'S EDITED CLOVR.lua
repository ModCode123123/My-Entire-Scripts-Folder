--GO TO LINE 198 FOR CLOVR SETTINGS!

local bodyTransparency = 0.4 --Change the transparency of your character (0 - 1)
...
--edited by AlgodooUser123
--|| Settings:
local StudsOffset = 0 -- Character height (negative if you're too high)
local Smoothness = 0 -- Character interpolation (0.1 - 1 = smooth - rigid)
...
local ViewportRange = 120 -- Maximum distance players are updated
...
local ChatHUDOutlineEnabled = true
local PointerOutlineEnabled = false
local ViewHUDOutlineEnabled = true
...
print("Thanks For Using AlgodooUser's Edited CLOVR!")
    end    
    --[[
...
--]]
    ...
    VirtualBody.Humanoid.WalkSpeed = 14
...
--]]
 ...
 --]]
        if VRReady then
            local Pointer = game:GetObjects("rbxassetid://11444662207")[1]
            Pointer.Parent = workspace
            Pointer.Beam.Enabled = false
            Pointer.Target.ParticleEmitter.Enabled = false
            if PointerOutlineEnabled == false then
Pointer.SelectionBox.Visible = false
            end
            local RenderStepped =
...
Permadeath = function()
    ...
    wait(game.Players.RespawnTime/1 + 0)
    warn("100%")
end
Respawn = function()
    local ch = game.Players.LocalPlayer.Character
...
ChatHUDFunc = function()
 ...
 --]]
    ...
    local Players = game:GetService("Players")
    local Client = Players.LocalPlayer
    local ChatHUD = game:GetObjects("rbxassetid://11444611419")[1]
   ...
   if ChatHUDOutlineEnabled == false then
    ChatHUD.Part:FindFirstChild("SelectionBox").Visible = false
   end
...
ViewHUDFunc = function()
 ...
 Variables
 --]]
    local ViewportRange = ViewportRange or 70
    local UserInputService = game:GetService("UserInputService")
    local RunService = game:GetService("RunService")
    local VRService = game:GetService("VRService")
    local VRReady = VRService.VREnabled
    ...
    local ViewHUD = script:FindFirstChild("ViewHUD") or game:GetObjects("rbxassetid://11450641160")[1]
   ...
   if ViewHUDOutlineEnabled == false then
    ViewHUD.Part.SelectionBox.Visible = false
   end
